...
function Leavesummary($conn, $Clientid, $user_id, $Employeeid, $Attendencedate) {
    try {
        ...
        $Firstname = "";
        $lastname = "";
        $logemp = "SELECT * FROM indsys1017employeemaster WHERE Clientid='$Clientid' and EmpActive='Active'  AND DATE(Date_Of_Joing) <='$Attendencedate' AND Employeeid NOT IN ( SELECT Employeeid FROM indsys1030empyearleavetakensummary WHERE Clientid='$Clientid' AND  AttendenceYear='$Attendanceyear' Employeeid IS NOT NULL)";
        //$logemp = "SELECT * FROM indsys1017employeemaster WHERE  EmpActive='Active' And Employeeid='$Employeeid' AND Clientid='$Clientid'  ";
        //printf($logemp);
        $logempall = mysqli_query($conn, $logemp);
        while ($row = mysqli_fetch_array($logempall)) {
            ...
            if ($Attendancemonth == $dateofjoiningmonth && $Attendanceyear == $dateofjoiningyear) {
                $TotalSickLeave = 0;
            ...
            $sqlsave = "INSERT IGNORE INTO indsys1030empyearleavetakensummary (Clientid,Employeeid,AttendenceYear,Title,Firstname,lastname,Userid,Addormodifydatetime,CausalleaveEligibility,UsedCasualleave,BalanceCausalLeave,SickleaveEligibility,UsedSickLeave,BalanceSickLeave)
values('$Clientid','$Employeeid','$Attendanceyear','$Title','$Firstname','$lastname','$user_id','$date','$TotalCausalLeave','$TakenCausalLeave','$BalanceCausalLeave','$TotalSickLeave','$TakenSickLeave','$BalanceSickLeave')";
            //printf($sqlsave);
            $resultsave = mysqli_query($conn, $sqlsave);
            if ($resultsave === true) {
            }
